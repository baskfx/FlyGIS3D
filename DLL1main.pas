...
function ShowBitmap(AHandle: THandle; ACaption: String): longint; StdCall;
procedure CloseBitmap(AFormRef: longint); stdcall;
...
function TDLL1frm.GetBM_X( MapInfo_X : real ):integer;
var
//  scale : real;
  w : integer;
begin
...
function TDLL1frm.GetBM_Y( MapInfo_Y : real ):integer;
var
//  scale : real;
  h : integer;
begin
...
function TDLL1frm.GetMI_X( Bitmap_X : integer ) : real;
var
//  scale : real;
  w : integer;
begin
...
function TDLL1frm.GetMI_Y( Bitmap_Y : integer ) : real;
var
//  scale : real;
  h : integer;
begin
...
procedure TDLL1frm.MyCircle ( x, y : real; r : integer );
begin
  Image1.Canvas.Ellipse ( GetBM_X ( x ) - r, GetBM_Y ( y ) - r,
                          GetBM_X ( x ) + r, GetBM_Y ( y ) + r );
end;

procedure TDLL1frm.ClearLastTraektoryLine;
begin
...
{    NegLine.Init ( Traektory2D[ points - 1 ].x, Traektory2D[ points - 1 ].y,
      Traektory2D[ points ].x, Traektory2D[ points ].y, DLL1frm.Image1.Canvas );
...
procedure TDLL1frm.ClearTraektory2D;
var
  i : integer;
begin
  points := TrPoints;
...
procedure TDLL1frm.Image1MouseDown(Sender: TObject;
...
begin
  if SetObscurePoint then begin
    ObscurePoint.x := DLL1frm.GetMI_X ( X );
    ObscurePoint.y := DLL1frm.GetMI_Y ( Y );
      ...
      if ( X <> Traektory2D [ TrPoints ].x ) and
        ...
        then begin
          ...
          if points = 1 then begin
            ...
            t2.x := X;
            t2.y := Y;
    {        NegLine.Init ( t1.x, t1.y, t2.x, t2.y, DLL1frm.Image1.Canvas );
            NegLine.Negate;
            NegLine.Done;}
            t1 := t2;
          end; // else
...
procedure TDLL1frm.FormPaint(Sender: TObject);
begin
  points := 0;
end;

procedure TDLL1frm.FormCreate(Sender: TObject);
begin
  ...
  Image1.Canvas.Pen.Mode := pmXOR;
  Image1.Canvas.Pen.Color := clWhite;
//  ShowMessage('BitmapForm created!');
end;


procedure TDLL1frm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  DLL1frm.Image1.AutoSize := false;
//  MainForm.MnuBitmap.Checked := false;
end;

procedure TDLL1frm.PopMnuEndingClick(Sender: TObject);
...
{    NegLine.Init ( Traektory2D [ points ].x, Traektory2D [ points ].y, oldX, oldY, DLL1frm.Image1.Canvas );
    NegLine.Negate;// ( DLL1frm.Image1.Canvas );
    NegLine.Done;}
    inc ( points );
    TrPoints := points;
...
{    NegLine.Init ( Traektory2D [ TrPoints - 1 ].x, Traektory2D [ TrPoints - 1 ].y,
      Traektory2D [ TrPoints ].x, Traektory2D [ TrPoints ].y, DLL1frm.Image1.Canvas );
    NegLine.Negate;// ( DLL1frm.Image1.Canvas );
    NegLine.Done;}
    Screen.Cursor := crDefault;
    points := 0;
...
procedure TDLL1frm.PopMnuDeleteClick(Sender: TObject);
begin
//  points := TrPoints;
  if points > 0 then begin
    Image1.Canvas.Pen.Mode := pmXOR;
...
procedure TDLL1frm.PopMnuUndoClick(Sender: TObject);
begin
  if points > 0 then begin
    Image1.Canvas.Pen.Mode := pmXOR;
    Image1.Canvas.Pen.Color := clWhite;
...
procedure TDLL1frm.Image1MouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
var
  A: BresenhamesLine;
  p, v: TPoint3D;
...
begin
  ...
  if ( TrPoints <> 0 ) and SetTraektory then begin
    ...
    Image1.Canvas.MoveTo ( Traektory2D[ points ].x, Traektory2D[ points ].y );
    Image1.Canvas.LineTo ( oldX, oldY );
{    NegLine.Init ( Traektory2D [ TrPoints ].x, Traektory2D [ TrPoints ].y, oldX, oldY, DLL1frm.Image1.Canvas );
    NegLine.Negate;// ( DLL1frm.Image1.Canvas );
    NegLine.Done;}
    Image1.Canvas.MoveTo ( Traektory2D [ points ].x, Traektory2D [ points ].y );
    Image1.Canvas.LineTo ( X, Y );
{    NegLine.Init ( Traektory2D [ TrPoints ].x, Traektory2D [ TrPoints ].y, X, Y, DLL1frm.Image1.Canvas );
    NegLine.Negate;// ( DLL1frm.Image1.Canvas );
    NegLine.Done;}
    GDIFlush;
    oldX := X;
...
//    scale := ( tmax - tmin ) / 0.008811 / MainForm.Width;
    v1.x := DLL1frm.GetMI_X ( trunc(VisiblePoint.x) );
    v1.y := DLL1frm.GetMI_Y ( trunc(VisiblePoint.y) );
...
///    end;
    v2.x := DLL1frm.GetMI_X ( X );
    v2.y := DLL1frm.GetMI_Y ( Y );
    k := ( CopyForm.Image1.Canvas.Pixels [ X , Y ] and $ff );
...
function ShowBitmap(AHandle: THandle; ACaption: String): longint;
var
  DLL1frm: TDLL1frm;
begin
  Application.Handle := AHandle;
  DLL1frm := TDLL1frm.Create(Application);

  DLL1frm.Caption := ACaption;
  DLL1frm.Show;
  Result := longint(DLL1frm);
end;

procedure CloseBitmap(AFormRef: longint);
begin
  if AFormRef > 0 then
    TDLL1Frm(AFormRef).Release;
end;
